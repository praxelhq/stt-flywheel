...
\textbf{Model weights.} The vasista22-base entity-dense LoRA adapters are released live on HuggingFace under Apache-2.0: \url{https://huggingface.co/Praxel/praxy-stt-te-rb}, \url{https://huggingface.co/Praxel/praxy-stt-hi-rb}, and \url{https://huggingface.co/Praxel/praxy-stt-ta-rb}. The vasista22 base remains under its upstream Apache-2.0 license; we redistribute only LoRA adapter weights. The Whisper-v3-base language-conditional adapters (Praxy-STT-r2) used in the Script Fidelity Rate analysis (\S\ref{sec:lang_conditional}) are not redistributed; their training recipe is fully specified in \S\ref{sec:lora} and is reproducible from public Whisper-v3 weights and the released EDSA corpus text.
